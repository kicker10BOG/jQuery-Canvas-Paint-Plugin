...
function CanvasPaintToolManager(owner) {
	...
	base.getTool = function(name) {
		...
		switch(name) {
			...
			case "rectangle" : return base.rectangle;
	...
	base.rectangle = {
		doAction : function(data) {
			...
			if (lbase.objects.length == 0 && !data.dragging) {data.order = 0;
				lbase.type = "rectangle";
				lbase.objects.push(new base.owner.OF.rectangleObject(data));
				lbase.currObj = lbase.objects[0];
			}
			else if (lbase.type == "rectangle") {
				//console.log("dragging");
				lbase.currObj.updateRectangle(data);
			}
		...
		select : function(data) {
			var rbase = base.rectangle;
			rbase.innerToolDiv = $("<div>").addClass("canvasPaintToolsInnerDiv");
			base.scrollingToolDiv.append(rbase.innerToolDiv);
			// fill color
			var fillColorDiv = $("<div>").addClass("canvasPaintToolOptionDiv").css("order", 0);
			rbase.innerToolDiv.append(fillColorDiv
				.append($("<label>").html("Fill Color: ")));
			rbase.fillColor = $("<input>").attr({
				type: "color",
				value: base.fillColor
			}).addClass("canvasPaintoutlineOption").on("input change", rbase.brushAdjusted);
			fillColorDiv.append(rbase.fillColor);
			// end fill color selector
			// range inputs
			var fillBrushRanges = $("<div>").css("Order", 1).addClass("canvasPaintToolOptionDiv");
			rbase.innerToolDiv.append(fillBrushRanges);
				// fill opacity
				fillBrushRanges.append($("<label>").html("Fill Opacity: "));
				rbase.fillOpacitySpan = $("<span>");
				fillBrushRanges.append(rbase.fillOpacitySpan).append("%<br>");
				rbase.fillOpacity = $("<input>").attr({
					type: "range",
					max: 100,
					min: 0,
					value: base.fillOpacity * 100
				}).addClass("canvasPaintSlider");
				fillBrushRanges.append(rbase.fillOpacity);
				rbase.fillOpacity.on("input change", rbase.brushAdjusted);
				// end fill opacity slider
			// end brush ranges
			// outline color
			var outlineColorDiv = $("<div>").addClass("canvasPaintToolOptionDiv").css("order", 0);
			rbase.innerToolDiv.append(outlineColorDiv
				.append($("<label>").html("outline Color: ")));
			rbase.outlineColor = $("<input>").attr({
				type: "color",
				value: base.outlineColor
			}).addClass("canvasPaintoutlineOption").on("input change", rbase.brushAdjusted);
			outlineColorDiv.append(rbase.outlineColor);
			// end outline color selector
			// range inputs
			var brushRanges = $("<div>").css("Order", 1).addClass("canvasPaintToolOptionDiv");
			rbase.innerToolDiv.append(brushRanges);
				// outline width
				brushRanges.append($("<label>").html("outline Width: "));
				rbase.outlineWidthSpan = $("<span>");
				brushRanges.append(rbase.outlineWidthSpan).append("<br>");
				rbase.outlineWidth = $("<input>").attr({
					type: "range",
					max: 25,
					min: 0,
					value: base.outlineWidth
				}).addClass("canvasPaintSlider");
				brushRanges.append(rbase.outlineWidth);
				rbase.outlineWidth.on("input change", rbase.brushAdjusted);
				brushRanges.append("<br>")
				// end outline width slider
				// outline opacity
				brushRanges.append($("<label>").html("outline Opacity: "));
				rbase.outlineOpacitySpan = $("<span>");
				brushRanges.append(rbase.outlineOpacitySpan).append("%<br>");
				rbase.outlineOpacity = $("<input>").attr({
					type: "range",
					max: 100,
					min: 0,
					value: base.outlineOpacity * 100
				}).addClass("canvasPaintSlider");
				brushRanges.append(rbase.outlineOpacity);
				rbase.outlineOpacity.on("input change", rbase.brushAdjusted);
				// end outline opacity slider
			// end brush ranges
			// brush preview
			rbase.brushToolPreview = $("<canvas>").attr({
				height: 150,
				width: 150
			}).addClass("canvasPaintBrushPreview");
			rbase.innerToolDiv.append($("<div>").addClass("canvasPaintToolOptionDiv canvasPaintCenter")
											.css("order", 3).append(rbase.brushToolPreview));
			rbase.brushAdjusted();
			// end brush preview
			
			rbase.icon.addClass("canvasPaintImgButtonSelected");
			base.toolNameSpan.html("Rectangle Tool");
			base.currTool = rbase;
		},
		
		unselect : function() {
			var rbase = base.rectangle;
			rbase.icon.removeClass("canvasPaintImgButtonSelected");
			rbase.innerToolDiv.remove();
		},
		
		addIcon : function(order) {
			var rbase = base.rectangle;
			rbase.icon = $("<img>").css("order", order).attr({
				src: "resources/rectangle.png",
				alt: "Rectangle",
				title: "Rectangle" 
			}).addClass("canvasPaintImgButton").click({tool:rbase}, base.setTool);
			base.drawingToolsGroup.append(rbase.icon);
		},
		
		brushAdjusted : function() {
			var rbase = base.rectangle;
			//console.log(e);
			base.fillColor = rbase.fillColor.val();
			base.fillOpacity = rbase.fillOpacity.val() / 100;
			base.outlineColor = rbase.outlineColor.val();
			base.outlineWidth = rbase.outlineWidth.val();
			base.outlineOpacity = rbase.outlineOpacity.val() / 100;
			rbase.outlineWidthSpan.html(base.outlineWidth);
			rbase.fillOpacitySpan.html(rbase.fillOpacity.val());
			rbase.outlineOpacitySpan.html(rbase.outlineOpacity.val());
			var ctx = rbase.brushToolPreview[0].getContext('2d');
			///console.log(ctx);
			ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
			ctx.fillStyle = base.fillColor;
			ctx.globalAlpha = base.fillOpacity;
			ctx.fillRect(30, 30, 90, 90);
			if (base.outlineWidth * base.outlineOpacity > 0) {
				ctx.beginPath();
				ctx.strokeStyle = base.outlineColor;
				ctx.lineWidth = base.outlineWidth;
				ctx.globalAlpha = base.outlineOpacity;
				ctx.moveTo(30 - base.outlineWidth / 2, 30  - base.outlineWidth / 2);
				ctx.lineTo(120 + base.outlineWidth / 2, 30  - base.outlineWidth / 2);
				ctx.lineTo(120 + base.outlineWidth / 2, 120 + base.outlineWidth / 2);
				ctx.lineTo(30 - base.outlineWidth / 2, 120 + base.outlineWidth / 2);
				ctx.closePath();
				ctx.stroke();
			}
		},
